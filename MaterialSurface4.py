...
path_rel = '/home/alexbou/CaNH100_relaxed/'  # path toward the relaxed slab.
path_froz = '/home/alexbou/CaNH100_frozen/'  # path toward the frozen slab.
path_bulk = '/home/alexbou/CaNH_rocksalt/'  # Path toward the bulk OUTCAR.
...
def PseudoBonds(atomList, elements):
    cutOffs = natural_cutoffs(atoms=atomList, mult=1.5)
...
def AddTile(tile, surfaceAtoms, cell):
    atomsPositions = []
    for i in tile:
        for atom in surfaceAtoms:
            if atom.index == i:
...
def RemoveSurface(surfaceAtoms, heights):
    surfaceAtomsCorrected = []
    for atom in surfaceAtoms:
        if str(round(atom.position[2], 3)) not in heights:
            surfaceAtomsCorrected.append(atom)
    return surfaceAtomsCorrected
...
for tile in initialTilesTop:
    AddTile(tile, cleanTopAtoms, cell=slab.get_cell())
...
'''
Remove some surface atoms if needed.
'''
errorAtoms = input('Is there any error in surface atoms? (y/n)\n')
while errorAtoms == 'y' or errorAtoms == 'yes':
    heightsInput = input('Choose atomic z-positions to delete (round to 3 digits):\n').split()
    heights = []
    for high in heightsInput:
        heights.append(high)
    plt.close(fig)

    # Remove atoms and regenerate new figure.
    topAtoms = RemoveSurface(topAtoms, heights=heights)
    cleanTopAtoms = Control(mat=slab, surfAtoms=topAtoms, side='top')
    initialTilesTop = PreAddedTiles(elements, cleanTopAtoms)
...
for tile in initialTilesBot:
    AddTile(tile, cleanBotAtoms, cell=slab.get_cell())
